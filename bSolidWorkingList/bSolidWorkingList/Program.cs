...
namespace bSolidWorkingList
{
	class Program
	{
		...
		{
			//const string path = @"C:\Users\ca01114\ownCloud\Customers\J & S CABINET DOOR\J&S Parametric Programs for Rover K COMPLETE SET for review (2)\Discontinuous Tool Paths\Discontinuous Slab Not Important";
			const string path = @".\";
			string[] filePaths = Directory.GetFiles(path, "*.lst");
			...
			foreach (var item in filePaths) {
				...
				using (StreamWriter file = new StreamWriter(path + "\\" + Path.GetFileNameWithoutExtension(item) + ".bwkl")) {
